...
<div class="block" id="menu" style="width:99%; margin-right:0.5%; text-align:right;">
	<?php if($_GET['fn']=="dashboard"){?>
		<img src="../../images/mea_hr/icon-create.png" class="btn-img" onclick="document.location='?controller=special&fn=add'" title="เพิ่มข้อมูล" />
	...
	<?php }else{?>
		...
		<?php if($_GET['detail'] != ''){ ?>
			<img src="../../images/mea_hr/icon-delete.png" class="btn-img" onclick="deleteData('<?php echo $_GET['detail']?>');" title="ลบข้อมูล" />
		<?php } ?>
		<img src="../../images/mea_hr/icon-cancel.png" class="btn-img" onclick="document.location='?controller=special&fn=dashboard'" title="ยกเลิก" />
	<?php } ?>
</div>